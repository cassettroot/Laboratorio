...
did_materials_bp = Blueprint('did_materials', __name__)

@did_materials_bp.route('/api/didactic-materials', methods=['GET'])
def get_materials():
    ...
    if similar_to:
        cursor.execute('SELECT * FROM didactic_materials WHERE id = ?', (similar_to,))
        base_item = cursor.fetchone()
        if not base_item:
            conn.close()
            return jsonify({"status": "error", "message": "Material didáctico base no encontrado"}), 404
    ...
    query = 'SELECT * FROM didactic_materials WHERE 1=1'
    params = []
...
@did_materials_bp.route('/api/didactic-materials/<int:item_id>', methods=['GET'])
def get_material(item_id):
    conn = get_db_connection()
    cursor = conn.cursor()
    cursor.execute('SELECT * FROM didactic_materials WHERE id = ?', (item_id,))
    row = cursor.fetchone()
    conn.close()

    if not row:
        return jsonify({"status": "error", "message": "Material didáctico no encontrado"}), 404
...
def create_material():
    ...
    if not name:
        return jsonify({"status": "error", "message": "El nombre del material didáctico es obligatorio"}), 400

    try:
        quantity = int(quantity_str)
    except ValueError:
        return jsonify({"status": "error", "message": "La cantidad debe ser un número entero válido"}), 400
    ...
    try:
        ...
        ''', (
            name, optional_vals['category'], quantity, optional_vals['location'],
        ...
        qr_path, qr_content = generate_qr('didactic_materials', record_id, custom_qr_content)

        cursor.execute('''
            UPDATE didactic_materials 
            SET qr_path = ?, qr_content = ? 
            WHERE id = ?
        ''', (qr_path, qr_content, record_id))

        log_creation(conn, user_responsible, 'didactic_materials', record_id)
        conn.commit()
        cursor.execute('SELECT * FROM didactic_materials WHERE id = ?', (record_id,))
...
def update_material(item_id):
    ...
    try:
        ...
        return jsonify({"status": "error", "message": "La cantidad debe ser un número entero"}), 400
    ...
    try:
        ...
        new_qr_content = data.get('qr_content', '').strip() or f"LAB-DIDACTIC_MATERIALS-{item_id}"
        qr_path = old_row['qr_path']
        qr_content = old_row['qr_content']
        
        if new_qr_content != old_row['qr_content']:
            qr_path, qr_content = generate_qr('didactic_materials', item_id, new_qr_content)
            new_data["qr_content"] = qr_content

        log_updates(conn, user_responsible, 'didactic_materials', item_id, old_row, new_data)
...
def delete_material(item_id):
    ...
    try:
        log_deletion(conn, user_responsible, 'didactic_materials', item_id)
        cursor.execute('DELETE FROM didactic_materials WHERE id = ?', (item_id,))
        conn.commit()
        conn.close()
        return jsonify({"status": "success", "message": "Material didáctico eliminado correctamente"})
    except Exception as e:
        conn.rollback()
        conn.close()
        return jsonify({"status": "error", "message": str(e)}), 500
