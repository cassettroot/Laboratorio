...
# We want to change the init_db function
match = re.search(r'def init_db\(\):.*?(?=def |\Z)', content, re.DOTALL)
if match:
    old_init = match.group(0)
    
    # We will redefine it to create all tables in all databases, and only users in the main DB
